...
#include "utility.h" //ty

// Compute distance between two points

double distanceBetween(Point p, Point q) {
  
  Point dif =  p-q; //ha we can do this because of the defined operators
...
void initPoint(struct Point *p, double xVal, double yVal) {
  ...
  //Precondition: We interpret *p as the address of a valid Point struct. xVal and yVal are necessarily doubles.
  //Postcondition: We expect the Point-struct at *p to take on the values of xVal and yVal in its members x, y.
  p->x = xVal;
  p->y = yVal;
  return;
}
...
bool boxesApproxEqual(Box b1, Box b2, double tolerance) {
  ...
  return pointsApproxEqual(b1.ul,b2.ul) 
...
double areaOfBox(Box b) {
  return b.area(); //it's in the struct lol
}
